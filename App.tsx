...
const getInitialState = () => {
    try {
        ...
        if (savedSessions && savedActiveIds) {
            ...
            const activeSessionIds = JSON.parse(savedActiveIds);
            
            // Fix duplicate or missing message IDs
            const usedIds = new Set<string>();
            const usedSessionIds = new Set<string>();
            for (const agent in sessions) {
                for (const session of sessions[agent]) {
                    // Fix duplicate or missing session IDs
                    if (!session.id || usedSessionIds.has(session.id)) {
    ...
    for (const agent of Object.values(Agent)) {
        ...
        sessions[agent] = [{
            id,
            title: 'New Chat',
            messages: [{ id: crypto.randomUUID(), role: Role.MODEL, content: agentIntroMessages[agent], agent }],
        }];
...
const App: React.FC = () => {
    ...
    const handleChatStream = useCallback(async (text: string) => {
        ...
        updateMessagesInSession(activeSession.id, (messages) => [...messages, userMessage, { id: crypto.randomUUID(), role: Role.MODEL, content: '', agent: activeAgent }]);
        ...
        try {
            ...
            updateMessagesInSession(activeSession.id, (messages) => messages.slice(0, -1).concat({ id: crypto.randomUUID(), role: Role.ERROR, content: finalError }));
            setError(finalError);
        } finally {
            setIsLoading(false);
    ...
    const handleImageGeneration = useCallback(async (prompt: string) => {
        ...
        const userMessage: Message = { id: crypto.randomUUID(), role: Role.USER, content: `/imagine ${prompt}` };
        ...
        try {
            ...
            const modelMessage: Message = { id: crypto.randomUUID(), role: Role.MODEL, content: '', imageUrls, agent: Agent.Default };
            ...
            updateMessagesInSession(activeSession.id, messages => [...messages, { id: crypto.randomUUID(), role: Role.ERROR, content: finalError }]);
            setError(finalError);
        } finally {
            setIsLoading(false);
            setGenerationType(null);
